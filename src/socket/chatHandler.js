...
const chatHandler = (io) => {
    ...
    io.on("connection", async (socket) => {
        ...
        // Helper function to join user to their conversations
        const joinUserToConversations = async (socket) => {
            try {
                ...
                logger.socket("auto_join_conversations", {
                    user: socket.user.name,
                ...
                logger.error("Auto join conversations error:", error);
                return 0;
            }
        };

        // Auto-join conversations on connection
        joinUserToConversations(socket)
            .then((count) => {
                console.log(
                    `✅ Auto-joined ${count} conversation rooms for ${socket.user.name}`,
                );
            })
            .catch((err) => {
        ...
        socket.on("join_conversations", async () => {
            try {
                const count = await joinUserToConversations(socket);
